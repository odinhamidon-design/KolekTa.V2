...
// Volume conversion: 1000 liters = 1 cubic meter (m³)
...
/**
 * Estimate bin volume based on capacity and fill level
 * @param {Object} bin - Bin object with capacity, currentLevel
 * @returns {number} Estimated volume in cubic meters (m³)
 */
function estimateBinVolume(bin) {
  const capacity = bin.capacity || 100; // liters
  const fillLevel = (bin.currentLevel || 0) / 100; // convert percentage to decimal
  const volumeLiters = capacity * fillLevel;

  // Convert liters to cubic meters (1000 liters = 1 m³)
  return Math.round((volumeLiters / LITERS_PER_CUBIC_METER) * 1000) / 1000; // m³, 3 decimal places
}
