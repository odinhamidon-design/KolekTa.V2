  ...
  while (improved && iterations < maxIterations) {
    ...
    for (let i = 0; i < route.length - 1; i++) {
      for (let j = i + 2; j < route.length; j++) {
        ...
        const currentDistance = calculateTotalDistance(route, depot);
...
function twoOptWithMatrix(locations, distanceMatrix, depot, initialOrder = null) {
  ...
  while (improved && iterations < maxIterations) {
    ...
    for (let i = 0; i < route.length - 1; i++) {
      for (let j = i + 2; j < route.length; j++) {
        const currentDistance = calcRouteDistance(route);
 ...
 *
 * @param {Array} locations - Array of {lat, lng, weight?} objects
 * @param {Object} options - { truckCapacity, capacityThreshold, distanceMatrix, depot }
...
function optimizeWithCapacity(locations, options = {}) {
  ...
  // Assign weights to locations
  const locationsWithWeight = locations.map((loc, i) => ({
    ...loc,
    weight: binWeights ? binWeights[i] : (loc.weight || loc.estimatedWeight || 20),
    originalIndex: i
  }));

  const trips = [];
  let remainingLocations = [...locationsWithWeight];
  let tripNumber = 0;
  ...
  while (remainingLocations.length > 0) {
    ...
    while (remainingLocations.length > 0) {
      ...
      for (let i = 0; i < remainingLocations.length; i++) {
        ...
        if (currentLoad + loc.weight <= maxLoad) {
    ...
    if (tripStops.length > 0) {
      ...
      trips.push({
        ...
        loadKg: Math.round(currentLoad * 10) / 10,
  ...
  const totalLoad = trips.reduce((sum, t) => sum + t.loadKg, 0);
...
async function optimizeRouteAsync(route, options = {}) {
  ...
  // Extract coordinates from route
  let coordinates = [];
  let binWeights = opts.binWeights || null;
  ...
  if (route.path && route.path.coordinates) {
    ...
    coordinates = route.locations.map(loc => [loc.lng, loc.lat]);
    if (!binWeights) {
      binWeights = route.locations.map(loc => loc.weight || loc.estimatedWeight || 20);
    }
  } else if (Array.isArray(route)) {
    coordinates = route;
  }
  ...
  if (opts.considerCapacity && binWeights) {
